/**
 * Shared Page Details Formatting Utility
 * Standardizes page selection, continuous range formatting, and document summaries across XeroxDosth.
 */
 ...
 * [2, 5, 9, 14] -> "2, 5, 9, 14"
 * 
 * If all pages of totalDocPages are selected (spanning 1 to totalDocPages), includes "All pages".
 */
...
/**
 * Returns formatted compact text strings for single-line or plain text contexts.
 */
export const formatDocSummaryLines = (doc = {}) => {
    ...
    details.categories.forEach(cat => {
        const pageText = cat.isAllPages ? `All pages · 1–${details.pageCount}` : `${cat.count} page(s) · ${cat.rangeText}`;
        lines.push(`${cat.colorMode} · ${cat.printSide} · ${pageText}`);
    });
